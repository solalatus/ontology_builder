...
Last updated: 2026-08-22T17:15:33.563Z
...
```
{
  "yaml": "competency_questions:\n  - text: Which air handler serves a given terminal unit?\n  - text: Which air handler serves a given plenum?\n  - text: Which air temperature sensors are tied to a given air handler?\n  - text: Which air temperature and control setpoints are tied to a given air handler?\n  - text: What HVAC equipment is physically located in a given building?\n  - text: What HVAC equipment is physically located on a given floor?\n  - text: What HVAC equipment is physically located in a given space?\n  - text: Which spaces are on a given floor?\n  - text: Which floors are in a given building?\n  - text: Which CO2 sensors measure outside air for a given air-handling system?\n  - text: Which CO2 sensors measure return air for a given air-handling system?\n  - text: What equipment is upstream of a given terminal unit?\n  - text: What equipment is upstream of a given plenum?\n  - text: If air temperature is off target, which setpoint should be checked?\n  - text: Which occupancy sensor is tied to a given space?\n  - text: Which occupancy sensor is tied to a given zone?\n  - text: Which cooling valves are part of a given air handler?\n  - text: Which heating valves are part of a given air handler?\n  - text: Which dampers are part of a given air handler?\n  - text: Which fans are part of a given air handler?\n  - text: Which filters are part of a given air handler?\n  - text: Which zones are served by a given thermostat?\n  - text: Which spaces are served by a given terminal unit?\n  - text: Which cooling devices are connected upstream of the cooling equipment being checked?\n  - text: Which heating devices are connected upstream of the heating equipment being checked?\n  - text: Under what conditions can the economizer be used instead of mechanical cooling?"
}
```
...
```
{
  "yaml": "actions:\n  enableEconomizerOperation:\n    input: AirHandler\n    effect: Enable economizer operation for the air handler.\n    verification: Confirm the air handler is operating with economizer enabled.\n  increaseCooling:\n    input: AirHandler\n    effect: Increase cooling for the air handler.\n    verification: Confirm cooling has increased for the air handler.\n  increaseHeating:\n    input: AirHandler\n    effect: Increase heating for the air handler.\n    verification: Confirm heating has increased for the air handler.\n  keepSteadyOperation:\n    input: AirHandler\n    effect: Keep the air handler in steady operation within the configured deadband.\n    verification: Confirm the air handler remains in steady operation without switching between heating and cooling.\n  verifyActiveControlAvailable:\n    input: Zone\n    effect: Verify whether active control is available for the occupied zone.\n    verification: Confirm the zone has active control available."
}
```
...
CONSISTENCY CHECK — 5 new problem(s) from this edit:
  [error] action-input-missing — Action "enableEconomizerOperation" has no input class. Give it the one class the action is fundamentally about.
  ...
  [error] action-input-missing — Action "keepSteadyOperation" has no input class. Give it the one class the action is fundamentally about.
  [error] action-input-missing — Action "verifyActiveControlAvailable" has no input class. Give it the one class the action is fundamentally about.
...
#   [error] action-input-missing — Action "enableEconomizerOperation" has no input class.
...
#   [error] action-input-missing — Action "keepSteadyOperation" has no input class.
#   [error] action-input-missing — Action "verifyActiveControlAvailable" has no input class.
...
```
{
  "yaml": "classes:\n  AirHandlingUnit:\n    aliases: [AHU, air handler, air handling unit]\n  Zone: {}\nactions:\n  enableEconomizerOperation:\n    input: AirHandlingUnit\n    effect: Enable economizer operation for the air handling unit.\n    verification: Confirm the air handling unit is operating with economizer enabled.\n  increaseCooling:\n    input: AirHandlingUnit\n    effect: Increase cooling for the air handling unit.\n    verification: Confirm cooling has increased for the air handling unit.\n  increaseHeating:\n    input: AirHandlingUnit\n    effect: Increase heating for the air handling unit.\n    verification: Confirm heating has increased for the air handling unit.\n  keepSteadyOperation:\n    input: AirHandlingUnit\n    effect: Keep the air handling unit in steady operation within the configured deadband.\n    verification: Confirm the air handling unit remains in steady operation without switching between heating and cooling.\n  verifyActiveControlAvailable:\n    input: Zone\n    effect: Verify whether active control is available for the occupied zone.\n    verification: Confirm the zone has active control available."
}
```

### Turn 7 — tool

Applied. Added 2, updated 5 existing item(s).
...
```
{
  "yaml": "classes:\n  AirHandlingUnit:\n    meaning: A piece of HVAC equipment that moves and conditions air for distribution through the building.\n  TerminalUnit:\n    meaning: A downstream device that regulates airflow or air temperature for the area it serves.\n  AirPlenum:\n    meaning: A section of the air-distribution path that receives air for supply, return, or exhaust.\n  Zone:\n    meaning: An area, or group of spaces, controlled together to maintain the same comfort conditions.\n  Space:\n    meaning: A physical room or defined area within the building.\n  Thermostat:\n    meaning: The control device that reads temperature and maintains the desired setpoint for a served zone.\n  TemperatureSetpoint:\n    meaning: The target temperature value the control system uses for heating, cooling, air temperature control, or deadband logic."
}
```
...
```
{
  "yaml": "classes:\n  Building:\n    meaning: The overall facility that contains the floors, spaces, zones, and HVAC equipment we manage.\n  Floor:\n    meaning: One level of the building that groups the spaces on that level.\n  AirTemperatureSensor:\n    meaning: A sensor that measures air temperature for monitoring or control.\n  OccupancySensor:\n    meaning: A sensor that indicates whether a space or area is occupied.\n  OutsideAirCO2Sensor:\n    meaning: A sensor that measures CO2 level in outside air.\n  ReturnAirCO2Sensor:\n    meaning: A sensor that measures CO2 level in return air coming back through the system.\n  CoolingValve:\n    meaning: A valve that controls cooling by adjusting cold-water flow through a coil.\n  HeatingValve:\n    meaning: A valve that controls heating by adjusting hot-water flow through a coil.\n  Damper:\n    meaning: A device that regulates or shuts off airflow in the air path.\n  Fan:\n    meaning: The part that moves air through the HVAC system.\n  Filter:\n    meaning: The part that removes unwanted material from the airstream.\n  Chiller:\n    meaning: Plant equipment that provides chilled water for cooling.\n  Boiler:\n    meaning: Plant equipment that provides hot water for heating.\n  PackagedHeatPump:\n    meaning: A self-contained unit that can provide both heating and cooling."
}
```
...
```
{
  "yaml": "relationships:\n  - name: feeds\n    from: AirHandlingUnit\n    to: TerminalUnit\n    meaning: The source equipment is upstream and supplies air, heating, or cooling to the downstream thing.\n  - name: feeds\n    from: AirHandlingUnit\n    to: AirPlenum\n    meaning: The source equipment is upstream and supplies air, heating, or cooling to the downstream thing.\n  - name: serves\n    from: TerminalUnit\n    to: Space\n    meaning: The equipment or device is responsible for conditioning or controlling the area it is assigned to.\n  - name: serves\n    from: Thermostat\n    to: Zone\n    meaning: The equipment or device is responsible for conditioning or controlling the area it is assigned to.\n  - name: contains\n    from: Floor\n    to: Space\n    meaning: The larger physical area includes the smaller area as part of its layout.\n  - name: contains\n    from: Building\n    to: Floor\n    meaning: The larger physical area includes the smaller area as part of its layout.\n  - name: hasPoint\n    from: AirHandlingUnit\n    to: AirTemperatureSensor\n    meaning: The equipment or area has a sensor or setpoint tied to it for monitoring or control.\n  - name: hasPoint\n    from: AirHandlingUnit\n    to: TemperatureSetpoint\n    meaning: The equipment or area has a sensor or setpoint tied to it for monitoring or control.\n  - name: hasPoint\n    from: Space\n    to: OccupancySensor\n    meaning: The equipment or area has a sensor or setpoint tied to it for monitoring or control.\n  - name: hasPoint\n    from: Zone\n    to: OccupancySensor\n    meaning: The equipment or area has a sensor or setpoint tied to it for monitoring or control.\n  - name: hasPart\n    from: AirHandlingUnit\n    to: CoolingValve\n    meaning: The equipment includes that component as one of its physical parts.\n  - name: hasPart\n    from: AirHandlingUnit\n    to: HeatingValve\n    meaning: The equipment includes that component as one of its physical parts.\n  - name: hasPart\n    from: AirHandlingUnit\n    to: Damper\n    meaning: The equipment includes that component as one of its physical parts.\n  - name: hasPart\n    from: AirHandlingUnit\n    to: Fan\n    meaning: The equipment includes that component as one of its physical parts.\n  - name: hasPart\n    from: AirHandlingUnit\n    to: Filter\n    meaning: The equipment includes that component as one of its physical parts.\n  - name: hasPoint\n    from: AirHandlingUnit\n    to: OutsideAirCO2Sensor\n    meaning: The equipment or area has a sensor or setpoint tied to it for monitoring or control.\n  - name: hasPoint\n    from: AirHandlingUnit\n    to: ReturnAirCO2Sensor\n    meaning: The equipment or area has a sensor or setpoint tied to it for monitoring or control.\n  - name: feeds\n    from: Chiller\n    to: CoolingValve\n    meaning: The source equipment is upstream and supplies air, heating, or cooling to the downstream thing.\n  - name: feeds\n    from: Boiler\n    to: HeatingValve\n    meaning: The source equipment is upstream and supplies air, heating, or cooling to the downstream thing.\n  - name: feeds\n    from: PackagedHeatPump\n    to: TerminalUnit\n    meaning: The source equipment is upstream and supplies air, heating, or cooling to the downstream thing.\n  - name: locatedIn\n    from: AirHandlingUnit\n    to: Building\n    meaning: The thing is physically installed within that building or space.\n  - name: locatedOn\n    from: AirHandlingUnit\n    to: Floor\n    meaning: The thing is physically installed on that floor or level.\n  - name: locatedIn\n    from: TerminalUnit\n    to: Space\n    meaning: The thing is physically installed within that building or space.\n  - name: locatedIn\n    from: Thermostat\n    to: Space\n    meaning: The thing is physically installed within that building or space."
}
```
...
CONSISTENCY CHECK — 2 new problem(s) from this edit:
  [warning] effect-verification-mismatch — Action "enableEconomizerOperation" verifies AirHandlingUnit.economizerStatus, but its effect never says that property changes. Either say in the effect what sets it, or verify something the effect actually does.
  [warning] unreachable-from-action-input — Action "verifyActiveControlAvailable" has input class Zone, and its precondition canVerifyActiveControlAvailable refers to Thermostat, which is only reachable against the direction of the relationships between them. Add or redirect a relationship so the agent can navigate from Zone to Thermostat. Rewording the text does not create the path it assumes.
...
#   [warning] effect-verification-mismatch — Action "enableEconomizerOperation" verifies AirHandlingUnit.economizerStatus, but its effect never says that property changes.
#   [warning] unreachable-from-action-input — Action "verifyActiveControlAvailable" has input class Zone, and its precondition canVerifyActiveControlAvailable refers to Thermostat, which is only reachable against the direction of the relationships between them.
...
```
{
  "yaml": "actions:\n  enableEconomizerOperation:\n    effect: The air handling unit economizer status indicates economizer operation so the unit reduces mechanical cooling."
}
```
...
```
{
  "yaml": "relationships:\n  - name: isServedBy\n    from: Zone\n    to: Thermostat\n    meaning: The zone is assigned to a thermostat responsible for controlling it.\n    aliases: [is controlled by]"
}
```
...
# CONSISTENCY CHECK — 1 outstanding problem(s) in the whole model:
#   [warning] inverse-pair — Zone --isServedBy--> Thermostat looks like the inverse of Thermostat --serves--> Zone. The profile is one directed relationship per real-world connection, never also its inverse.
...
```
{
  "yaml": "relationships:\n  - name: isServedBy\n    from: Zone\n    to: Thermostat\n    meaning: null\n    aliases: []\nactions:\n  verifyActiveControlAvailable:\n    verification: Confirm a thermostat that serves the zone has normal status."
}
```
