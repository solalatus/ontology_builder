...
import { withPage, addNodeViaDblClick, APP_URL } from "./lib/page.mjs";
...
// the real response back untouched -- see the file header for why this
// hop exists. Returns the array of real response bodies seen, in order, for
// tests that want to inspect what the live API actually sent back.
function forwardToRealOpenAi(page, url) {
  const responses = [];
...
async function connectAgentLive(page, apiKey) {
...
async function sendChatMessage(page, text) {
  ...
  await page.waitForFunction(() => !window.__kg.agent.isSending(), null, { timeout: 60000 });
}
...
// resource log, not an application error. Same pattern as the mocked
// suites' own withPageAllowingResourceErrors, needed here because the 401
// this test deliberately triggers is a genuine live response, not a mock.
